...
| [design/002-artifacts-and-wrap-up.md](./design/002-artifacts-and-wrap-up.md) | PROPOSED: give a ticket's product an identity so R1 is provable directly rather than by proxy — which is what lets the merge machinery leave the domain — and make wrap-up a declared kind so a ticket that merges nothing can finish. |
