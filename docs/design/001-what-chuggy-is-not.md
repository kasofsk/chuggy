...
**No fabric is named.** Which thing actually runs a task — Nomad, Kubernetes, a bare daemon — is a property of a deployment, not of chuggy. It is chosen per cluster behind one port, and more than one may exist at once: an on-prem fleet with macOS nodes and a cloud cluster have different right answers and there is no reason to make them agree. *Why this is an absence rather than an omission:* the refusal is what the controllers-and-CRDs entry below argues for at length. If a fabric were named in the core, the core would drift toward its vocabulary, and the refinement obligation exists precisely to stop that. So the core names none, and each adapter states its own guarantees and the obligations it must supply itself.
