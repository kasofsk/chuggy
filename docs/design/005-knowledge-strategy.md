...
S8 has landed, so the first retro is due: it runs over the record of the pure-core build — the git history, the pull requests, the ticket, and whatever of the 2026-08-15/16 sessions still exists when it runs. Its expected outputs are candidate rules, candidate prunes, and the removal bar above. House rule 15 and `.chug/tasks/check-knowledge.sh` are the calibration: both are observer-shaped changes that happened inline, because the observer did not exist yet, and the loop's outputs match their voice and placement.
