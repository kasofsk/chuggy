-- migration 1: the database baseline

  DO $$
  BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'chuggy_ticket_service') THEN
      CREATE ROLE chuggy_ticket_service NOLOGIN;
    END IF;
...
;
  ...
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'chuggy_api') THEN
      CREATE ROLE chuggy_api NOLOGIN;
    END IF;
  EXCEPTION WHEN duplicate_object THEN
    NULL;
  END
...
;
  ...
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'chuggy_boundary_owner') THEN
      CREATE ROLE chuggy_boundary_owner NOLOGIN;
    END IF;
  EXCEPTION WHEN duplicate_object THEN
    NULL;
  END
...
;
  ...
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'chuggy_selector_service') THEN
      CREATE ROLE chuggy_selector_service NOLOGIN;
    END IF;
  EXCEPTION WHEN duplicate_object THEN
    NULL;
  END
...
;
  ...
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'chuggy_selector_control') THEN
      CREATE ROLE chuggy_selector_control NOLOGIN;
    END IF;
  EXCEPTION WHEN duplicate_object THEN
    NULL;
  END
...
;
  ...
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'chuggy_selector_review') THEN
      CREATE ROLE chuggy_selector_review NOLOGIN;
    END IF;
  EXCEPTION WHEN duplicate_object THEN
    NULL;
  END
...
;
  ...
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'chuggy_scheduler') THEN
      CREATE ROLE chuggy_scheduler NOLOGIN;
    END IF;
  EXCEPTION WHEN duplicate_object THEN
    NULL;
  END
...
;
  ...
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'chuggy_finalizer') THEN
      CREATE ROLE chuggy_finalizer NOLOGIN;
    END IF;
  EXCEPTION WHEN duplicate_object THEN
    NULL;
  END
...
;
  ...
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'chuggy_worker_plane') THEN
      CREATE ROLE chuggy_worker_plane NOLOGIN;
    END IF;
  EXCEPTION WHEN duplicate_object THEN
    NULL;
  END
...
;
  ...
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'chuggy_configuration_importer') THEN
      CREATE ROLE chuggy_configuration_importer NOLOGIN;
    END IF;
  EXCEPTION WHEN duplicate_object THEN
    NULL;
  END
...
INSERT INTO public.selector_runtime_settings
   ...
   VALUES (1, 1, 'Running', 'ApprovalRequired', 'Select at most one currently dispatchable ticket. Use the supplied project view and advisory operational context. Prefer work that unblocks other tickets, respect explicit urgency and dependencies, and wait when evidence or safe capacity is insufficient. Use only authorized selector tools and record the evidence used for the decision.',
     '{"limits":{"tokensPerDecision":17525063,"concurrentDecisions":4,"selectionsPerMinute":60,"toolCallsPerDecision":200,"dispatchesPerDecision":3,"inputBytesPerDecision":1048576,"millisecondsPerDecision":900000,"candidatePagesPerDecision":1},"toolAllowlist":["Glob","Grep","Read","mcp__chuggy__list_tickets","mcp__chuggy__read_ticket","mcp__chuggy__read_draft","mcp__chuggy__list_drafts","mcp__chuggy__list_configurations","mcp__chuggy__read_configuration","mcp__chuggy__read_decision_log","mcp__chuggy__read_refusals","mcp__chuggy__read_ticket_refusals","mcp__chuggy__read_projects","mcp__chuggy__read_lead","mcp__chuggy__read_lead_transcript","mcp__chuggy__list_executions","mcp__chuggy__read_execution","mcp__chuggy__read_run_transcript","mcp__chuggy__read_operation","mcp__chuggy__list_threads","mcp__chuggy__read_thread","mcp__chuggy__read_thread_transcript","mcp__chuggy__initialize_draft","mcp__chuggy__file_dependent","mcp__chuggy__revise_draft","mcp__chuggy__delete_draft","mcp__chuggy__release_draft","mcp__chuggy__dispatch","mcp__chuggy__refuse","mcp__chuggy__lift","mcp__chuggy__set_attention","mcp__chuggy__set_handoff_note","mcp__chuggy__set_planning_intent"],"modelAllowlist":["*"],"operationalContextMaxAgeMs":30000}', now());;
...
-- migration 2: worker-pool

  DO $$
  BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'chuggy_pool_plane') THEN
      CREATE ROLE chuggy_pool_plane NOLOGIN;
    END IF;
  EXCEPTION WHEN duplicate_object THEN
    NULL;
  END
  $$
;
GRANT USAGE ON SCHEMA public TO chuggy_pool_plane;
GRANT SELECT ON TABLE public.schema_migration TO chuggy_pool_plane;
...
CREATE TABLE public.worker_pool_registration_token (
       ...
       FOREIGN KEY(tenant,project) REFERENCES public.project(tenant,project));
GRANT SELECT,INSERT,DELETE ON TABLE public.worker_pool TO chuggy_api;
GRANT SELECT,INSERT,DELETE ON TABLE public.worker_pool_registration_token TO chuggy_api;
GRANT UPDATE(redeemed_at) ON TABLE public.worker_pool_registration_token TO chuggy_api;
GRANT SELECT(tenant,project,pool,capabilities,principal) ON TABLE public.worker_pool TO chuggy_pool_plane;
GRANT SELECT(tenant,project,execution,status,placement,placement_backoff_from,requirement_value)
       ON TABLE public.execution TO chuggy_pool_plane;
GRANT UPDATE(placement_backoff_from) ON TABLE public.execution TO chuggy_pool_plane;
GRANT EXECUTE ON FUNCTION public.execution_status_move_is_legal(before text, after text) TO chuggy_pool_plane;
GRANT SELECT(tenant,project,execution,attempt,generation,recovery_epoch,state,
       ...
       ON TABLE public.execution_attempt TO chuggy_pool_plane;
GRANT UPDATE(lease_owner,lease_expires_at,capability_secret_digest,pool,assignment,pool_refusal)
       ON TABLE public.execution_attempt TO chuggy_pool_plane;
GRANT SELECT(tenant,project,lifecycle) ON TABLE public.project TO chuggy_pool_plane;
GRANT SELECT ON TABLE public.recovery_epoch TO chuggy_pool_plane;

-- migration 3: the handoff phases leave the schema
DO $$
...
-- migration 4: the accounts leave the schema
DO $$
       DECLARE holders text;
       BEGIN
         SELECT string_agg(relation, ', ' ORDER BY relation) INTO holders FROM (
...
-- migration 5: the finalizer choice, the revoke cascade and the combinator leave
DO $$
       DECLARE holders text;
       BEGIN
         SELECT string_agg(relation, ', ' ORDER BY relation) INTO holders FROM (
...
-- migration 6: the phases, reasons and resume points take the package's names
ALTER TABLE public.ticket_projection
       DROP CONSTRAINT ticket_projection_phase_is_known,
       DROP CONSTRAINT ticket_projection_reason_is_known,
       DROP CONSTRAINT ticket_projection_resume_is_known;
...
-- migration 7: a finalization nothing can carry out escalates, and says what held it
ALTER TABLE public.ticket_projection
       DROP CONSTRAINT ticket_projection_reason_is_known,
...
GRANT UPDATE(hold_kind) ON TABLE public.finalization_request TO chuggy_boundary_owner;
GRANT UPDATE(hold_passes) ON TABLE public.finalization_request TO chuggy_boundary_owner;
GRANT UPDATE(held_since) ON TABLE public.finalization_request TO chuggy_boundary_owner;
CREATE FUNCTION public.record_finalization_hold(in_tenant text, in_project text, in_request text, in_kind text, in_claim_owner text, in_claim_generation bigint, in_request_generation bigint, in_recovery_epoch text) RETURNS TABLE(result text, hold_passes integer)
    ...
    AS $$
     ...
     END $$;;
ALTER FUNCTION public.record_finalization_hold(in_tenant text, in_project text, in_request text, in_kind text, in_claim_owner text, in_claim_generation bigint, in_request_generation bigint, in_recovery_epoch text) OWNER TO chuggy_boundary_owner;
REVOKE ALL ON FUNCTION public.record_finalization_hold(in_tenant text, in_project text, in_request text, in_kind text, in_claim_owner text, in_claim_generation bigint, in_request_generation bigint, in_recovery_epoch text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.record_finalization_hold(in_tenant text, in_project text, in_request text, in_kind text, in_claim_owner text, in_claim_generation bigint, in_request_generation bigint, in_recovery_epoch text) TO chuggy_finalizer;
CREATE OR REPLACE FUNCTION public.submit_finalization_result(in_tenant text, in_project text, in_request text, in_attempt text, in_outcome text, in_failure_kind text, in_request_generation bigint, in_recovery_epoch text, in_operation text, in_authority_subject text) RETURNS TABLE(result text, operation text, ordinal bigint)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'pg_catalog', 'public', 'pg_temp'
    AS $$
...
GRANT SELECT(tenant,project,ticket,authorizing_seq,hold_kind,hold_passes,held_since)
       ON TABLE public.finalization_request TO chuggy_api;

-- migration 8: a parked ticket carries one escalation and the evidence for it
DO $$
       BEGIN
...
ALTER TABLE public.ticket_projection
       ...
       ADD CONSTRAINT ticket_projection_escalation_is_known CHECK ((escalation = ANY (ARRAY['NoEscalation'::text, 'WorkFailureEscalated'::text, 'WorkExecutionUnavailableEscalated'::text, 'EvaluationFailureEscalated'::text, 'EvaluationBlockedEscalated'::text, 'FinalizationUnavailableEscalated'::text]))),
...
GRANT SELECT(escalation) ON TABLE public.ticket_projection TO chuggy_api;
GRANT UPDATE(escalation) ON TABLE public.ticket_projection TO chuggy_ticket_service;
GRANT SELECT(escalation_evidence) ON TABLE public.ticket_projection TO chuggy_api;
GRANT UPDATE(escalation_evidence) ON TABLE public.ticket_projection TO chuggy_ticket_service;
GRANT SELECT(completion_operation) ON TABLE public.execution TO chuggy_ticket_service;
GRANT SELECT(blocked_reason) ON TABLE public.execution TO chuggy_ticket_service;
ALTER TABLE public.native_action
       DROP CONSTRAINT native_action_reason_check;
ALTER TABLE public.native_action RENAME COLUMN reason TO escalation;
ALTER TABLE public.native_action
       ADD CONSTRAINT native_action_escalation_check CHECK (((escalation = ANY (ARRAY['NoEscalation'::text, 'WorkFailureEscalated'::text, 'WorkExecutionUnavailableEscalated'::text, 'EvaluationFailureEscalated'::text, 'EvaluationBlockedEscalated'::text, 'FinalizationUnavailableEscalated'::text])) OR ((state <> 'Open'::text) AND (escalation = 'DependencyRevoked'::text))));
CREATE OR REPLACE FUNCTION public.decision_event_is_valid(event jsonb) RETURNS boolean
    LANGUAGE plpgsql IMMUTABLE
    AS $$
     DECLARE tag text; value jsonb; item jsonb;
...
-- migration 9: a work set is one task, so a ticket authors no fan-out
DO $$
       BEGIN
         IF EXISTS (SELECT FROM public.journal_entry) THEN
           RAISE EXCEPTION 'this image replays no journal written before it; empty the journal with deploy/rig/wipe-tickets.sql first'
...
-- migration 10: a task is named by its cycle, its stage, its generation and its evaluator
DO $$
       BEGIN
         IF EXISTS (SELECT FROM public.journal_entry) THEN
           RAISE EXCEPTION 'this image replays no journal written before it; empty the journal with deploy/rig/wipe-tickets.sql first'
...
GRANT SELECT(cycle) ON TABLE public.execution_request_task TO chuggy_api;
GRANT SELECT(generation) ON TABLE public.execution_request_task TO chuggy_api;
GRANT SELECT(evaluator) ON TABLE public.execution_request_task TO chuggy_api;
...
-- migration 11: a stage names the evaluators it runs, and its key is its place
DO $$
       BEGIN
         IF EXISTS (SELECT FROM public.journal_entry) THEN
           RAISE EXCEPTION 'this image replays no journal written before it; empty the journal with deploy/rig/wipe-tickets.sql first'
...
-- migration 12: a completion carries the report its task terminated under
DO $$
       BEGIN
         IF EXISTS (SELECT FROM public.journal_entry) THEN
           RAISE EXCEPTION 'this image replays no journal written before it; empty the journal with deploy/rig/wipe-tickets.sql first'
...
ALTER TABLE public.execution
       ...
       ADD CONSTRAINT execution_outcome_is_known CHECK (((outcome IS NULL) OR (outcome = ANY (ARRAY['Passed'::text, 'Failed'::text, 'Blocked'::text, 'ProcessFailed'::text]))));
CREATE OR REPLACE FUNCTION public.decision_event_is_valid(event jsonb) RETURNS boolean
    LANGUAGE plpgsql IMMUTABLE
    AS $$
     DECLARE tag text; value jsonb; item jsonb; entry jsonb; place bigint;
...
CREATE OR REPLACE FUNCTION public.submit_task_completion(in_tenant text, in_project text, in_execution text, in_ticket bigint, in_task bigint, in_source_effect integer, in_outcome text, in_manifest text, in_manifest_digest text, in_reason text, in_operation text, in_authority_subject text) RETURNS TABLE(result text, operation text, ordinal bigint)
    ...
    AS $$
     ...
     BEGIN
       ...
       ELSIF in_outcome = 'ProcessFailed' THEN
         report := jsonb_build_object('type', 'TerminalFailureReport', 'value',
           jsonb_build_object('evidence', bound.task,
             'kind', 'ProcessFailure'));
...
-- migration 13: a ticket is released as a definition, and runs at the source it was dispatched from
DO $$
       BEGIN
         IF EXISTS (SELECT FROM public.journal_entry) THEN
           RAISE EXCEPTION 'this image replays no journal written before it; empty the journal with deploy/rig/wipe-tickets.sql first'
...
CREATE TABLE public.ticket_definition (
    ...
    CONSTRAINT ticket_definition_material_is_bounded CHECK (((jsonb_typeof(definition) = 'object'::text) AND (length((definition)::text) <= 65536))),
...
GRANT SELECT,INSERT ON TABLE public.ticket_definition TO chuggy_ticket_service;
GRANT SELECT ON TABLE public.ticket_definition TO chuggy_boundary_owner;;
GRANT SELECT,INSERT ON TABLE public.ticket_source TO chuggy_ticket_service;
GRANT SELECT,INSERT ON TABLE public.ticket_source TO chuggy_boundary_owner;
GRANT SELECT ON TABLE public.ticket_source TO chuggy_scheduler;;
GRANT SELECT(tenant) ON TABLE public.ticket_source TO chuggy_api;
GRANT SELECT(project) ON TABLE public.ticket_source TO chuggy_api;
GRANT SELECT(ticket) ON TABLE public.ticket_source TO chuggy_api;
GRANT SELECT(source) ON TABLE public.ticket_source TO chuggy_api;
GRANT SELECT(repository) ON TABLE public.ticket_source TO chuggy_api;
GRANT SELECT(commit) ON TABLE public.ticket_source TO chuggy_api;
GRANT SELECT(ref) ON TABLE public.ticket_source TO chuggy_api;
GRANT SELECT(tenant) ON TABLE public.journal_entry TO chuggy_boundary_owner;
GRANT SELECT(project) ON TABLE public.journal_entry TO chuggy_boundary_owner;
GRANT SELECT(entry) ON TABLE public.journal_entry TO chuggy_boundary_owner;
DROP INDEX public.journal_entry_release_ticket;
CREATE INDEX journal_entry_release_ticket ON public.journal_entry USING btree (tenant, project, (
CASE
    WHEN (entry IS JSON OBJECT) THEN ((((entry)::jsonb -> 'event'::text) -> 'value'::text) -> 'id'::text)
    ELSE NULL::jsonb
END)) WHERE (
CASE
    WHEN (entry IS JSON OBJECT) THEN (((entry)::jsonb -> 'event'::text) ->> 'type'::text)
...
CASE command_tag
    ...
    ELSE true
END) NOT VALID;
CREATE FUNCTION public.command_reference(value jsonb) RETURNS boolean
    LANGUAGE sql IMMUTABLE
    AS $$
       SELECT command_integer(value) AND value > '0'::jsonb
     $$;
ALTER FUNCTION public.command_reference(value jsonb) OWNER TO chuggy_boundary_owner;
REVOKE ALL ON FUNCTION public.command_reference(value jsonb) FROM PUBLIC;
CREATE FUNCTION public.command_task_definition(definition jsonb) RETURNS boolean
    LANGUAGE sql IMMUTABLE
    AS $$
       SELECT jsonb_typeof(definition) = 'object'
          AND command_reference(definition->'workload')
          AND command_reference(definition->'inputs')
          AND command_reference(definition->'executionRequirements')
          AND command_reference(definition->'resultContract')
     $$;
ALTER FUNCTION public.command_task_definition(definition jsonb) OWNER TO chuggy_boundary_owner;
REVOKE ALL ON FUNCTION public.command_task_definition(definition jsonb) FROM PUBLIC;
CREATE OR REPLACE FUNCTION public.decision_event_is_valid(event jsonb) RETURNS boolean
    LANGUAGE plpgsql IMMUTABLE
    AS $$
     DECLARE tag text; value jsonb; item jsonb; entry jsonb; place bigint;
     ...
     BEGIN
       ...
       IF tag = 'Dispatch' THEN
         RETURN jsonb_typeof(value) = 'object'
           AND command_reference(value->'ticket')
           AND command_reference(value->'source');
       END IF;
       IF tag = 'TaskDone' THEN
         task := value->'task'; constructor := COALESCE(task->>'type', '');
         report := value->'report'; arm := COALESCE(report->>'type', '');
         ...
         END IF;
         IF arm = 'TerminalFailureReport' THEN
           RETURN command_reference(report->'value'->'evidence')
             AND COALESCE(report->'value'->>'kind', '')
               IN ('ProcessFailure', 'ExecutionUnavailableFailure');
         END IF;
         IF arm NOT IN ('WorkResultReport', 'EvaluationResultReport')
            OR obligation->'task' IS DISTINCT FROM task
            OR NOT command_task_definition(obligation->'definition')
            OR NOT command_reference(obligation->'contextRef')
            OR jsonb_typeof(produced->'resultRef') <> 'object'
            OR NOT command_reference(produced->'resultRef'->'manifest')
            OR NOT command_reference(produced->'resultRef'->'digest')
            OR NOT command_reference(produced->'resultRef'->'schema') THEN
           RETURN false;
         END IF;
         IF arm = 'WorkResultReport' THEN
           RETURN command_reference(report->'value'->'acceptedSourceRef');
         END IF;
       ...
       IF tag <> 'CreateTicket' OR jsonb_typeof(value) <> 'object'
          ...
          OR NOT command_reference(value->'id')
          OR NOT command_reference(value->'content')
          OR NOT command_reference(value->'finalizationConfiguration')
          OR NOT command_task_definition(value->'workConfiguration')
          OR jsonb_typeof(value->'dependencies') IS DISTINCT FROM 'array'
          OR jsonb_typeof(value->'evaluationPlan') IS DISTINCT FROM 'object'
          OR jsonb_typeof(value->'evaluationPlan'->'stages')
             IS DISTINCT FROM 'array' THEN
         RETURN false;
       END IF;
       dependencies := value->'dependencies';
       stages := value->'evaluationPlan'->'stages';
       ...
       FOR item IN SELECT element FROM jsonb_array_elements(dependencies) AS elements(element) LOOP
         IF NOT command_reference(item) THEN RETURN false; END IF;
       END LOOP;
       FOR item, place IN
         SELECT element, ordinality
       ...
       LOOP
         IF jsonb_typeof(item) <> 'object'
            OR NOT command_reference(item->'key')
            OR jsonb_typeof(item->'evaluators') <> 'array' THEN
           RETURN false;
         END IF;
         IF (item->>'key')::bigint <> place
            OR jsonb_array_length(item->'evaluators') < 1 THEN
         ...
         FOR entry IN SELECT element
               FROM jsonb_array_elements(item->'evaluators') AS elements(element) LOOP
           IF jsonb_typeof(entry) <> 'object'
              OR NOT command_reference(entry->'key')
              OR NOT command_task_definition(entry->'task') THEN
             RETURN false;
           END IF;
         END LOOP;
         IF (SELECT count(DISTINCT element->'key')
       ...
       ELSE
         SELECT (j.entry::jsonb)->'event'->'value' INTO released
          ...
          WHERE j.tenant = in_tenant AND j.project = in_project
            ...
            AND (CASE WHEN j.entry IS JSON OBJECT
                      THEN j.entry::jsonb->'event'->'value'->'id' END)
         ...
         ELSE
           ...
           ELSIF EXISTS (SELECT FROM ticket_source t
             ...
             RETURN QUERY SELECT 'SourceUnrecorded'::text, NULL::text, NULL::bigint;
             RETURN;
           ELSE
