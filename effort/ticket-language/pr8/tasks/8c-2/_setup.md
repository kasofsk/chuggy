## Setup (every 8c-2 task)

You are launched in your own git worktree cut from `origin/main` (b6e5b4fb, "Merge pull request #735"; confirm `git log -1 --oneline`). Bash is pinned to that worktree: do not `cd` into, or `git -C`, any other checkout. Absolute-path reads with the Read tool are fine.

- Branch: `git checkout -b <your branch>` (A, S) or `git fetch origin <branch> && git checkout <branch>` (B), once, as setup.
- `node_modules`: `ln -s /Users/david/chuggy/node_modules node_modules` (its lockfile equals b6e5b4fb's). If that is refused, `npm ci` at the root. **Never** `npm ci` under `ui/`.
