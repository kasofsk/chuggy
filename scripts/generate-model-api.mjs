...
const output = argument("out") ?? join(root, "src/generated/model-api.ts");
...
for (const entry of ordered) {
  lines.push(`export type ${entry.name} = ${ts(entry.type, entry.name)};`);
  lines.push(
...
if (globalThis.process.argv.includes("--check")) {
  ...
  if (current !== generated) {
    globalThis.process.stderr.write(
      "generate-model-api: src/generated/model-api.ts is stale; regenerate it\n",
    );
