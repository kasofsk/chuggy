 ...
 *
 * THE READS FILTER TO THE KINDS THE STREAM CARRIES, in the server rather than
 * after the rows arrive. The log is a superset of the publication log and still
 * holds kinds nothing writes any more, plus `Project`, which no route answers
 * one entry of; filtering here means a batch of those never displaces carried
 * rows from a bounded read, and the cursor still advances past them because it
 * follows the sequence of the last row returned.
 *
...
const carriedKinds: readonly string[] = [...projectStreamKinds];

function changeKind(value: string): ProjectStreamKind {
  const found = projectStreamKinds.find((known) => known === value);
  if (found === undefined)
    throw new Error(`project change row: unstreamed kind ${value}`);
  return found;
}
    ...
    since: async (after, limit) => {
      const found = await pool.query<ChangeRow>(
        sql`SELECT sequence::text AS seq,tenant,project,kind,resource
             FROM project_change
            WHERE sequence>${after} AND kind=ANY(${carriedKinds})
      ...
      const found = await pool.query<ChangeRow>(
        sql`SELECT sequence::text AS seq,tenant,project,kind,resource
            ...
            WHERE tenant=${partition.tenant} AND project=${partition.project}
              AND sequence>${sequence} AND kind=ANY(${carriedKinds})
            ORDER BY sequence LIMIT ${limit}`,
      );
      return found.rows.map(changeRow);
    },
