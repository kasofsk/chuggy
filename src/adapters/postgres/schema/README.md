...
issue #180 requires every new mutable relation to state who owns it and
which role writes it, its key and identity, what changes it, and how
unfinished work is found, so each one states them here rather than in a doc
that would drift from the DDL beside it.
