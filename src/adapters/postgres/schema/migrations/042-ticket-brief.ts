...
const draftReviseSignature =
  "text,text,bigint,bigint,text,text,text,text[],text,text,text";

/**
 * The intent is the one column here a newline belongs in, because it renders
 * as lines; every other control character is refused wherever it appears.
 */
const briefRelations = [
  `CREATE TABLE draft_brief (
     tenant text NOT NULL, project text NOT NULL, ticket bigint NOT NULL,
     intent text NOT NULL, branch text,
     PRIMARY KEY (tenant,project,ticket),
     CONSTRAINT draft_brief_belongs_to_draft FOREIGN KEY (tenant,project,ticket)
       REFERENCES draft (tenant,project,ticket),
     CONSTRAINT draft_brief_intent_is_bounded
       CHECK (length(intent) BETWEEN 1 AND ${briefIntentCharsMax}),
     CONSTRAINT draft_brief_intent_renders
       CHECK (translate(intent,chr(10),' ') !~ '[[:cntrl:]]'),
     CONSTRAINT draft_brief_branch_is_a_ref
       CHECK (branch IS NULL OR (length(branch) BETWEEN 1 AND ${briefBranchCharsMax}
         AND branch LIKE '${briefBranchPrefix}%' AND branch !~ '[[:cntrl:]]'))
   )`,
  `CREATE TABLE draft_brief_link (
     tenant text NOT NULL, project text NOT NULL, ticket bigint NOT NULL,
     ordinal integer NOT NULL, url text NOT NULL,
     PRIMARY KEY (tenant,project,ticket,ordinal),
     CONSTRAINT draft_brief_link_belongs_to_brief FOREIGN KEY (tenant,project,ticket)
...
const briefWriters = [
  `DROP FUNCTION ${draftCreateFunction}(text,text,text,text,bigint,text,text,text)`,
  `CREATE FUNCTION ${draftCreateFunction}(in_tenant text,in_project text,in_configuration text,
      ...
      in_intent text,in_links text[],in_branch text,in_kind text,in_subject text)
     ...
     BEGIN
       ...
       INSERT INTO draft_brief (tenant,project,ticket,intent,branch)
         VALUES (in_tenant,in_project,minted,in_intent,in_branch);
     ...
     END $$`,
  `DROP FUNCTION ${draftReviseFunction}(text,text,bigint,bigint,text,text,text,text)`,
  `CREATE FUNCTION ${draftReviseFunction}(in_tenant text,in_project text,in_ticket bigint,
     ...
     BEGIN
       ...
       INSERT INTO draft_brief (tenant,project,ticket,intent,branch)
         VALUES (in_tenant,in_project,in_ticket,in_intent,in_branch)
...
export const migration042: Migration = {
  ...
  statements: [
    ...briefRelations,
    ...briefWriters,
    `GRANT SELECT,INSERT,UPDATE ON draft_brief TO ${boundaryOwnerRole}`,
    `GRANT SELECT,INSERT,DELETE ON draft_brief_link TO ${boundaryOwnerRole}`,
    `GRANT SELECT ON draft_brief,draft_brief_link
       TO ${apiRole},${ticketServiceRole},${schedulerRole}`,
  ],
};
