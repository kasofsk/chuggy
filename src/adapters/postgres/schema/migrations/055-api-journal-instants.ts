 ...
 * there; `seq` and `committed_at` are what it selects and `tenant` and
 * `project` are what it filters on. The grant is column-level and read-only,
 * and the API role's writes to this table remain the ones it never had.
 *
 ...
 *
 * THE COLUMN IS TEXT AND MAY NOT BE JSON. `./journal.ts` returns a refusal for
 * a stored row it cannot parse rather than raising, so a row that is not JSON is
 * a state this table admits — and an index or a read that cast it unguarded
 * would turn that row into a write the server rejects and a page nobody can
 * load. The `CASE` is what keeps the cast off such a row, and the read repeats
 * the same expression because that is what lets this index answer it.
 */
    ...
    `CREATE INDEX journal_entry_release_ticket ON journal_entry
       (tenant, project,
        ((CASE WHEN entry IS JSON OBJECT
               THEN entry::jsonb->'event'->'value'->>'ticket' END)::bigint))
