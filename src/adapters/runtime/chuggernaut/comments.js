...
export class Removal {
  ...
  removed;
  preserved;
  constructor(source, removed, preserved) {
    this.source = source;
    this.removed = removed;
    this.preserved = preserved;
  }
}
export function supported(path) {
...
function directive(text, syntax) {
...
async function parse(language, source, path) {
...
async function syntax_spans(path, source) {
  const suffix = extname(path),
...
function lex_spans(path, source) {
  const suffix = extname(path),
...
async function spans(path, source) {
  const suffix = extname(path);
  if (suffix === ".qnt")
    return [
      ...source.matchAll(/"(?:\\.|[^"\\])*"|\/\/[^\r\n]*|\/\*[\s\S]*?\*\//g),
...
async function syntax_shape(path, source) {
  const suffix = extname(path),
    tree = await parse(PARSERS[suffix], source, path);
  try {
