 ...
 * an append is only safe if the lease, the fencing epoch and the expected head
 * are all rechecked in the same transaction as the insert. Three ports invite
 * an implementation that checks ownership in one call and appends in the next,
 ...
 * old one, because rewiring it is the project decision transaction — slice I2
 * — and a slice may not reach forward into the next one.
 */
...
/**
 * What an append found. `StaleHead` carries the head the caller should have
 * observed, because a writer that reloads from it can retry without a second
 * round trip to ask.
 */
export type Appended =
  | { readonly appended: "Committed"; readonly head: number }
  | { readonly appended: "StaleHead"; readonly head: number }
  | { readonly appended: "Fenced"; readonly fencingEpoch: number }
  | { readonly appended: "NotActive"; readonly lifecycle: Lifecycle };
...
export interface ProjectStore {
  ...
  /**
   * Appends one entry if and only if the head is still `lease.head` and the
   * lease is still valid by database time, then advances the head.
   */
