...
/**
 * One page of the log. `nextAfter` is present on a full page, which is where a
 * further one may stand; a reader that follows it to an empty page has reached
 * the end, and no over-fetch is spent to tell it so a page sooner.
 */
...
export function checkedSelectorHistoryLimit(limit: number): number {
  if (
...
/** The one method the log needs, which is why the API holds no more of the store than this. */
export type SelectorHistoryStore = Pick<SelectorStateStore, "history">;
...
export function selectorHistory(
  ...
  return {
    read: async (principal, partition, after, limit) => {
      ...
      const page = checkedSelectorHistoryLimit(limit);
      const decisions = (await store.history(partition, after, page)).map(
        selectorDecisionSummary,
      );
      ...
      return {
        result: "Found",
        decisions,
        ...(decisions.length < page || last === undefined
          ? {}
