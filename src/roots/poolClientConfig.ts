 ...
 * one that goes stale is the one nobody reads. Kubernetes is the only fabric
 * this tree places on, and the tag is a union of one so that a second arrives
 * as a member here rather than as a branch everywhere.
 */

import { z } from "zod";

import type { KubernetesPoolPlacementConfig } from "../adapters/kubernetes/poolPlacement.ts";
import type { ClientCredentialsConfig } from "../adapters/http/clientCredentials.ts";
...
export interface PoolClientConfig {
  readonly tokens: ClientCredentialsConfig;
...
export function poolClientConfig(
  ...
  return {
    tokens: {
      ...
      responseBytesMax: poolClientPositive(
        environment,
        "TOKEN_BYTES_MAX",
        64 * 1024,
      ),
      responseReadsMax: poolClientPositive(environment, "TOKEN_READS_MAX", 64),
      refreshMarginMs: poolClientPositive(
        environment,
        "TOKEN_REFRESH_MARGIN_MS",
        60_000,
      ),
      mintCooldownMs: poolClientPositive(
        environment,
        "TOKEN_MINT_COOLDOWN_MS",
        1_000,
      ),
    },
    plane: {
