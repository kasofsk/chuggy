 ...
 * else — the obligation, the task key, the ticket, the repository, the inputs,
 * the result contract, the context, every credential and every content
 * reference — the harness fetches from `callbackUrl` under `bearer`, or never
 * leaves the orchestrator at all. That is why this is thin, and why it is here:
 * a third party implements the wire, so it is written where the wire is.
 *
 * NOTHING HERE IDENTIFIES THE WORK. `assignment` is opaque because a pool that
 * could read a task key would learn the tenant's ticket structure, and
 * capability tokens are opaque because a pool may translate one into a node
...
/**
 * A settled no becomes ticket evidence; a temporary no is the pool's own
