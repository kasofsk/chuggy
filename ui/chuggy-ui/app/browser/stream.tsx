 ...
 *
 * A change frame stales the key its kind and resource name and offers its
 * representation to the lists registered here — folded in where a screen's own
 * fold can read it, read again where the frame only says the entry is stale.
 * The stream is reopened when the partition changes and when the session's
 * token generation does, because both make the connection in flight the wrong
 * one.
 */
...
/**
 * EVERY INVALIDATION KEEPS `cancelRefetch`'S DEFAULT, which is true and
 * restarts the read in flight; joining it instead issues no request and clears
 * the invalidation on settling, so a frame arriving mid-read would be swallowed
 * and the entry would keep an answer given before that frame's change existed.
 *
 * THE RESTART IS ITSELF CONDITIONAL, and that condition is the limit this
 * console accepts: an entry with no data yet joins its in-flight first read
 * whatever the flag says, so a frame arriving during that first read is
 * swallowed and the bounded fallback is what corrects it.
 */
function applyListRefresh(
  ...
  switch (refresh.refresh) {
    case "Fold":
      ...
      return;
    case "Reread":
      void client.invalidateQueries({ queryKey: registered.key, exact: true });
      return;
    case "RereadNamed":
...
function applyCommand(
  ...
  switch (command.command) {
    case "StaleResource":
      void client.invalidateQueries({ queryKey: command.key, exact: true });
      return;
    case "DropResource":
...
function useStreamConnection(
  ...
  useEffect(() => {
    const opened = openProjectStream(
      { fetch: transport, bearer: () => holder.bearer(), sleepMs, nowMs },
      { tenant, project },
      {
        onEvent: (event) => {
 ...
 * it would abort and restart the loop at every rung — while the loop sleeps
 * before its first refetch, so no such restart would ever reach one.
 */
