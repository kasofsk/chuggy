...
import { executionStandingTone, phaseTone } from "../core/tones.ts";
...
function TicketRow(props: {
  ...
  return (
    <tr>
      ...
      <td>
        {row.executionRead === "Joined" &&
        row.executionStatus !== undefined &&
        status !== undefined ? (
          <Pill
            tone={executionStandingTone(
              row.executionStatus,
          ...
          <span className="text-ink-3">
            {ticketRowExecutionCell(row, status)}
          </span>
        )}
      </td>
      <td>
        {row.runsOn === undefined ? (
          ...
          <Pill tone="neutral">
            <Tooltip text={row.runsOn.title}>
              <span className="max-w-aside min-w-0 inline-block truncate align-bottom">
                {row.runsOn.text}
              </span>
            </Tooltip>
          </Pill>
