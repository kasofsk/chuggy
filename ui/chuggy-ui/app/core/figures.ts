 ...
 * minutes, hours and days. A clock that ran backwards clamps to no time
 * elapsed rather than a negative one. The one rounding an elapsed time is
 * read with, so `freshnessLabel` and `agoFigure` both take it from here.
 */
