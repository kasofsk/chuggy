 ...
 * turn is out and nothing has been said for it yet, absent once every
 * exchange has settled or answered, the strip is never drawn where there is
 * no composer to run above, and the sprite states neither a colour nor a
 * style of its own so the theme and the served policy both hold.
 */
...
test("a running exchange already carrying an answer draws no engine", () => {
  const answered = exchangeOf({
    standing: { standing: "Running", state: "Claimed" },
    answer: "here is what it found",
  });
  const view = render(
    <Conversation
      exchanges={[answered]}
