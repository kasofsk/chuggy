/**
 * The shell is drawn around every screen, so its footer is the one thing every
 * page carries regardless of which panel is mounted inside it.
 */
...
function served(url: string): Response {
  ...
  if (url.includes("/tenants/"))
    return answer({ partition: atlas, sequence: 1, tickets: [] });
  return answer({ projects: [atlas] });
}

test("the shell carries the copyright footer on every screen it wraps", async () => {
