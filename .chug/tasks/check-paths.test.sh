...
printf '%s\n' 'x' > "$R/src/real.ts"
printf '%s\n' 'The layer lives at src/missing-layer/ <!-- intent -->' > "$R/docs/design/004-plan.md"
git -C "$R" add -A
run_in "$R"
...
mkdir -p "$R/docs/design" "$R/src"
printf '%s\n' 'x' > "$R/src/real.ts"
printf '%s\n' 'The layer lives at src/missing-layer/' > "$R/docs/design/004-plan.md"
git -C "$R" add -A
run_in "$R"
...
printf '%s\n' 'x' > "$R/src/real.ts"
printf '%s\n' 'The layer lives at src/missing-layer/ <!-- intent -->' > "$R/docs/notes.md"
git -C "$R" add -A
run_in "$R"
