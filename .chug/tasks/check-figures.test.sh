...
#
# THE NEGATIVE CASES ARE THE POINT OF THIS SUITE. This gate refuses a shape
# legitimate prose is full of, so every allowed kind of figure gets a case
# proving it stays silent: a threshold, a version pin, a date, a run-time
# derivation, a small enumeration, an ordered list, a fenced block.
#
# Fixtures are built with printf rather than a heredoc: a heredoc body carrying
# a `#` would be a comment line in THIS file, which the gate reads, and the
# suite would reject itself.
#
...
# A numeral taking a measure noun, with an adjective between them.
...
# A DURATION IS A MEASUREMENT AT ANY SIZE. Below the floor a number is
...
# Markdown is prose throughout — there is no comment marker to hide behind.
...
# A small number counting what the sentence has just listed.
...
# THE ORDERED LIST. Its leading numeral is a label, and the noun that follows
# belongs to the sentence rather than to the number.
doc_saying '2. Read the files it touches in full, not just the hunks.'
check "an ordered list marker is silent" 0 "$RC" "0 finding(s)"

# A fenced block is code, wherever it is quoted from.
doc_saying 'Before:' '```sh' '# the model run is ~50s and fifteen lines long' '```'
check "a fenced block is silent" 0 "$RC" "0 finding(s)"

# A small number in front of a countable noun describes a shape rather than
# measuring the tree.
...
# THE MODEL IS IN THE DEFAULT CORPUS, and a Quint `///` is a comment marker
# like any other, so its executable lines are not read at all.
fresh_repo "$R"
mkdir -p "$R/model"
printf '%s\n' '#!/bin/sh' 'exit 0' > "$R/gate.sh"
printf '%s\n' '/// the eight witness modules' > "$R/model/example.qnt"
git -C "$R" add -A
run_in "$R"
check "a model doc comment is in the default corpus" 1 "$RC" "eight — a spelled quantity"
fresh_repo "$R"
mkdir -p "$R/model"
printf '%s\n' 'val note = "eight witness modules"' > "$R/model/example.qnt"
git -C "$R" add -A
run_in "$R"
check "a model line that is not a comment is not read" 0 "$RC" "0 finding(s)"

# A design doc may carry a measurement, dated and reproducible, so the
# directory is left alone. Naming one scans it anyway, which is what keeps the
# exemption from being a place to hide a figure.
fresh_repo "$R"
...
# A path with a space in it is scanned like any other. The corpus is split on
# newlines for exactly this reason.
fresh_repo "$R"
