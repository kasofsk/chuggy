...
if [ "\$sub" = typecheck ] && [ "\${STUB_IDENTITY_FAILURE-0}" = 1 ]; then
  case "\$*" in
    *model/identity.qnt*) echo 'error: identity type error'; exit 1 ;;
  esac
fi
...
export STUB_PROCESSING_FAILURE
run_in_repo
unset STUB_PROCESSING_FAILURE
check "an adopted processing model failure rejects the gate" 1 "$RC" "model/application/project-decision-processing/processing_tests.qnt failed"

model_repo 0.32.0
STUB_IDENTITY_FAILURE=1
export STUB_IDENTITY_FAILURE
run_in_repo
unset STUB_IDENTITY_FAILURE
check "a root model type error rejects the gate" 1 "$RC" "model/identity.qnt: does not typecheck"
...
# A SUITE THAT RAN AND FAILED IS A FINDING, and what it said is kept: a suite
...
# TWO IS NOT A PASS, AND NEITHER IS ONE. Quint exits 1 for a failed suite and
# for a module it cannot parse alike, and a crash leaves on a
