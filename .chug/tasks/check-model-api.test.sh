...
printf 'stale\n' >"$WORK/modelTypes.ts"
set +e
(cd "$ROOT" && node ./scripts/generate-model-api.ts --check \
...
set +e
(cd "$ROOT" && node ./scripts/generate-model-api.ts \
	--out-schemas="$WORK/model-api.ts" --out-types="$WORK/modelTypes.ts") >"$OUT" 2>&1
printf 'stale\n' >"$WORK/modelTypes.ts"
(cd "$ROOT" && node ./scripts/generate-model-api.ts --check \
	--out-schemas="$WORK/model-api.ts" --out-types="$WORK/modelTypes.ts") >"$OUT" 2>&1
...
set +e
(cd "$ROOT" && node ./scripts/generate-model-api.ts --check --out="$WORK/model-api.ts") >"$OUT" 2>&1
RC=$?
set -e
