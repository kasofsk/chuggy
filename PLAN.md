...
| `s2b-eval-gate-desk` | The other half: the remaining 8 `*In` (`reducibleEvalIn`, `wrapUpStartablesIn`, `wrapUpStartableIn`, `leaseFreeIn`, `holdingIn`, `doneIn`, `retryablesIn`, `retryableIn`) with `wrapUpOutcomes`, `resumeCharge`, `leaseOf`, `withWrapUpObs`, `completeTicket`; deciders `decideEvalStageReduce`, `decideWrapUpStart`, `decideDequeue`, `decideWrapUpResolve`, `decideCompleteDuplicate`, `decideRevalFail`, `decideOpRetry` (the remaining 7, including the guarded-unreachable `operator-retry-unreachable` arm, implemented exactly as the model writes it). The 56-run mirror of `chuggy_test.qnt` closes with this slice. | s2a-guards-work | landed PR #23 |
