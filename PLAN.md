...
| `sync-wrap-up-identifiers` | Absorb upstream PR #57: the model renamed its own land-family identifiers (`escLanding`→`escWrapUp`, `jLand`→`jWrapUp`, `quietLandDeterministicTest`→`quietWrapUpDeterministicTest`, both attribution run names) — the exact negative roster PR #60 kept because the model kept it. The slice's demonstration is sweep 2's machinery firing on real drift for the first time: the witness-run roster reds on the merged-but-unmirrored tree. Mirrors, test titles, manifest run fields and the fixture follow; PR #60's keep-arguments for these names are rewritten with them. | sync-wrap-up-vocabulary | landed PR #61 |
| `s3-conformance-machinery` | Sweep 3's conformance-alarm findings (issue #62): the sum-type CONSTRUCTOR rosters (S3-1) and the `*In` enablement roster (S3-2), both in the twelfth roster's idiom; a meta-completeness guard that reds when the model grows any `type`/`*In` no roster claims (the cheap slice of Option B that ends the recurrence); `effectGaps`' missing wiring proof (S3-9); the three shipped comments that assert a control which does not exist. Owns src/tools/{corpus,verify}.ts, src/spine/{coverage,itf}.ts, verify.test.ts. | (sweep 3) | in progress |
| `s3-gates` | Sweep 3's gate/checker findings (issue #62): `check-model.sh` classifying a quint crash as a spec violation (S3-6) and never running modules smuggled into the unit-suite file (S3-7); the test-runner glob silently skipping `.mts`/moved test files (S3-8); the purity stage unable to run half its own config and the loader bans proved by no suite case (S3-5); the readable single-writer routes `Reflect.get`/`eval` (S3-3/S3-4); the comment corrections that stop overclaiming what the boundary encloses. Owns .chug/tasks/{check-model,check-ts}.sh (+tests), eslint.purity.config.js, .dependency-cruiser.mjs. | (sweep 3) | in progress |
