...
test("a ticket's title is the first line of its intent, and no line after it", () => {
  const intent = asBriefIntent(
    "Fix the importer.\nIt drops rows on a repeated key.",
  );
  assert.equal(briefTitle(intent), "Fix the importer.");
});
