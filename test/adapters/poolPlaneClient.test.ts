...
import { clientCredentialsTokens } from "../../src/adapters/http/clientCredentials.ts";
import {
...
test("the token request names the audience, without which the token carries none", async () => {
  const sent: { body: string; authorization: string } = {
    body: "",
    authorization: "",
  };
  const tokens = clientCredentialsTokens(tokenSettings, (_input, init) => {
    sent.body = sentBody(init);
    sent.authorization = sentHeader(init, "authorization");
    return Promise.resolve(
      answered(
        200,
        JSON.stringify({ access_token: "minted", expires_in: 3_600 }),
      ),
    );
  });
...
test("a grant the issuer refused is a denial and everything else is an outage", async () => {
  for (const [status, expected] of [
    ...
    const tokens = clientCredentialsTokens(tokenSettings, () =>
      Promise.resolve(answered(status, "{}")),
    );
    assert.equal(
...
test("a granted answer with no token and an issuer that did not answer are both outages", async () => {
  const empty = clientCredentialsTokens(tokenSettings, () =>
    Promise.resolve(answered(200, JSON.stringify({ expires_in: 60 }))),
  );
  assert.equal((await empty.acquire()).acquired, "Unavailable");
  const unreachable = clientCredentialsTokens(tokenSettings, () =>
    Promise.reject(new Error("connection refused")),
  );
  assert.equal((await unreachable.acquire()).acquired, "Unavailable");
});

test("a granted answer with no expiry is one this side stops using at once", async () => {
  const tokens = clientCredentialsTokens(tokenSettings, () =>
    Promise.resolve(answered(200, JSON.stringify({ access_token: "minted" }))),
  );
