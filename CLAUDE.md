...
- **A design doc has a mutable head, and a body you may edit.** The head — title, `Status:`, the tables — is rewritten freely, so knowing where things stand costs one read. The body is where a decision is argued, and the discipline it will eventually carry is that it only grows, by appended `## Correction — YYYY-MM-DD` sections, because the decision as argued is the record. That ratchet is not on yet, and turning it on early costs more than it buys: while the tree is still being built the vocabulary moves under the prose, and a Correction section announcing that a word is now spelled differently tells a reader nothing a diff would not. So edit the body when the edit changes what a claim is *called*, and append a Correction when it changes what the claim *says*. The distinction is the whole rule and it is the author's to make. The ratchet arrives when the implementation a doc argues for has landed — after that, rewriting the argument loses the record of why the tree looks the way it does. There is no syntactic boundary between head and body; read for where the argument starts.
